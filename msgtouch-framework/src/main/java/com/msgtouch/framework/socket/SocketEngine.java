...
public class SocketEngine {
    ...
    private static SocketServerSetting getSocketServerSetting(ApplicationContext applicationContext){
        ...
        if(environment.containsProperty(Constraint.PORPERTIES_TCP_SERVER_BOSSTHREADSIZE)){
            setting.bossThreadSize=Integer.parseInt(environment.getProperty(Constraint.PORPERTIES_TCP_SERVER_BOSSTHREADSIZE));
        }
        if(environment.containsProperty(Constraint.PORPERTIES_TCP_SERVER_CMDTHREADSIZE)){
            setting.cmdThreadSize=Integer.parseInt(environment.getProperty(Constraint.PORPERTIES_TCP_SERVER_CMDTHREADSIZE));
        }
        if(environment.containsProperty(Constraint.PORPERTIES_TCP_SERVER_WORKERTHREADSIZE)) {
            setting.workerThreadSize = Integer.parseInt(environment.getProperty(Constraint.PORPERTIES_TCP_SERVER_WORKERTHREADSIZE));
        }

        if(!environment.containsProperty(Constraint.PORPERTIES_TCP_SERVER_PORT)){
            throw new IllegalArgumentException("SocketServerEngine msgtouch.tcp.server.port is needed ");
        }
        setting.port=Integer.parseInt(environment.getProperty(Constraint.PORPERTIES_TCP_SERVER_PORT));
